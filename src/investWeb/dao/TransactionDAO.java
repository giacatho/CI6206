...
import investWeb.servlet.Portfolio;
...
public class TransactionDAO extends AbstractDAO {
    ...
    public ArrayList<Transaction> getTransactionList(String userId)
    {
	    ...
	    {	
	        ...
	        String sql = new String("SELECT A.*, B.name FROM tb_trans A, tb_stock B WHERE A.userid=? and A.symbol=B.symbol order by symbol,date");
    ...
    private void tradeStmt(Transaction trans, String sql)
    {
    	try
    	{
    		...
    		if(trans.getDate()!=null&&!trans.getDate().isEmpty())
    		{
    		  try
    		  {	
    			SimpleDateFormat format = new SimpleDateFormat("MM/dd/yyyy");
    		    java.util.Date parsed = format.parse(trans.getDate());
   		        txnDate = new java.sql.Date(parsed.getTime());
    		  }catch(ParseException pe)
    		  {
    			  logger.error(pe.fillInStackTrace());
    			  txnDate = new Date(Calendar.getInstance().getTimeInMillis());
    		  }
    		}
    		else
    		{
